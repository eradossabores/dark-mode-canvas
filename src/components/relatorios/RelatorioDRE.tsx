...
export default function RelatorioDRE({ factoryId, dateRange }: Props) {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [vendasR, comprasR, pagR, comR, ajudaR] = await Promise.all([
  ...
  const lucroOperacional = lucroBruto - data.despesasTotal;
  ...
  const linhasSimples: Linha[] = [
    ...
    { label: "(-) Despesas Operacionais", valor: -(data.despesasTotal + data.comissoes + data.ajudaCusto) },
