  ...
  return (
    <div id={exportId} className="space-y-4">
      <div className="flex flex-wrap items-end gap-4 justify-between">
        ...
        <ExportButtons
          onExportPDF={() => exportToPDF(exportId, `Comissões - ${factoryName}`)}
          onExportExcel={() => exportToExcel(ranking.map(r => ({
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KpiCard title="Total Vendido" value={formatBRL(totalGeral)} icon={<DollarSign className="h-4 w-4" />} />
        <KpiCard title="Taxa de Comissão" value={`${comissaoConfig}%`} icon={<Percent className="h-4 w-4" />} />
        <KpiCard title="Total Comissões" value={formatBRL(totalComissao)} icon={<DollarSign className="h-4 w-4 text-green-500" />} />
      </div>

      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Comissão por Vendedor</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
