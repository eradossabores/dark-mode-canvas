...
import { Plus, Trash2, ShoppingCart, Pencil, Eye, CalendarIcon, X, Truck, Package, History, CalendarClock, Receipt, CopyPlus, Sparkles, Repeat, Check, ChevronsUpDown } from "lucide-react";
...
export default function Vendas() {
  ...
  async function handleSubmit() {
    ...
    const temBebida = bebidaItens.some(bi => bi.bebida_id && bi.quantidade > 0);
    if (itensValidos.length === 0 && !temGeloCubo && !temBebida) return toast({ title: "Adicione ao menos um produto à comanda", variant: "destructive" });
    ...
    for (let i = 0; i < itensValidos.length; i++) {
      // Skip brindes (preco_auto === false means manually set, including free items)
