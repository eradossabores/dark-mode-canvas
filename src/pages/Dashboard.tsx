...
export default function Dashboard() {
  ...
  async function loadStats() {
    try {
      ...
      let vendasQuery = (supabase as any).from("vendas").select("total, valor_pago, created_at, status, clientes(nome)");
  ...
  return (
    <div className="relative space-y-4 sm:space-y-6 pb-8 w-full overflow-x-hidden">
       ...
       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-4 sm:mb-6">
         {cards.map((c: any, idx: number) => (
           <div key={c.title} className="relative overflow-hidden rounded-xl border border-transparent p-0.5 opacity-0 animate-fade-in flex" style={{ animationDelay: `${idx * 80}ms`, animationFillMode: "forwards" }}>
             ...
             />
             <Card
               className="relative cursor-pointer rounded-xl border-0 bg-[hsl(var(--kpi-surface))] text-[hsl(var(--kpi-surface-foreground))] transition-all hover:scale-[1.03] hover:shadow-lg flex flex-col w-full min-h-[160px] group"
             ...
             >
               <CardHeader className="flex flex-row items-center justify-between pb-2 shrink-0 space-y-0">
                 <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-[hsl(var(--kpi-surface-muted-foreground))] line-clamp-1 opacity-80">{c.title}</CardTitle>
                 <div className={`p-1.5 rounded-lg bg-black/20 border border-white/5 transition-colors group-hover:border-primary/30`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6 opacity-0 animate-fade-in" style={{ animationDelay: "500ms", animationFillMode: "forwards" }}>
       ...
       {/* Seção Principal: Metas e Inteligência */}
       <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 mb-4 sm:mb-6 opacity-0 animate-fade-in" style={{ animationDelay: "600ms", animationFillMode: "forwards" }}>
         <div className="relative rounded-xl border-[0.75px] border-border p-0.5 lg:col-span-2">
           ...
           <div className="relative h-full"><MetaVendas factoryId={factoryId} /></div>
         </div>
         <div className="relative rounded-xl border-[0.75px] border-border p-0.5 lg:col-span-2">
           <GlowingEffect spread={15} glow disabled={false} proximity={32} inactiveZone={0.3} borderWidth={3} />
           <div className="relative h-full"><EstoqueInteligente factoryId={factoryId} /></div>
         </div>
       </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6 opacity-0 animate-fade-in" style={{ animationDelay: "700ms", animationFillMode: "forwards" }}>
         <div className="relative rounded-xl border-[0.75px] border-border p-0.5 lg:col-span-2">
           <GlowingEffect spread={15} glow disabled={false} proximity={32} inactiveZone={0.3} borderWidth={3} />
           <div className="relative h-full"><ComparativoMensal factoryId={factoryId} /></div>
         </div>
         <div className="relative rounded-xl border-[0.75px] border-border p-0.5">
           <GlowingEffect spread={15} glow disabled={false} proximity={32} inactiveZone={0.3} borderWidth={3} />
