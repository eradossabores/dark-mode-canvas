...
import {
  CalendarDays, Plus, Trash2, Save, ArrowLeft,
...
export default function PlanoSemanal() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const [saboresRes, estoqueRes, vendasRes, planosRes] = await Promise.all([qSabores, qEstoque, qVendas, qPlanos]);
      ...
      (vendasRes.data || []).forEach((v: any) => {
        ...
        if (dt >= trintaDias) v30d[v.sabor_id] = (v30d[v.sabor_id] || 0) + v.quantidade;
      });
      setVendas7d(v7d);
  ...
  async function loadPlan(id: string, nome: string) {
    setPlanoId(id);
    setPlanoNome(nome);
    let q = (supabase as any).from("plano_semanal_itens").select("*").eq("plano_id", id);
    const { data } = await q;
  ...
  function duplicarPlano() {
    ...
    setTimeout(() => {
      setPlanoId(null);
      setPlanoNome("Plano Semanal (cópia)");
    }, 100);
  ...
  function getStatusBadge(projetado: number, media7d: number) {
    if (media7d === 0) return null;
    const coberturaDias = projetado / media7d;
    if (coberturaDias > 14) return <Badge variant="outline" className="bg-amber-500/10 text-amber-600 text-[10px]"><AlertTriangle className="h-3 w-3 mr-1" />Excesso</Badge>;
    if (coberturaDias < 3) return <Badge variant="outline" className="bg-red-500/10 text-red-600 text-[10px]"><TrendingDown className="h-3 w-3 mr-1" />Risco</Badge>;
    ...
    <div className="space-y-4 pb-24">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Distribua a produção nos dias da semana</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {DIAS_SEMANA.map(dia => {
          ...
          return (
            <Card key={dia.value} className="border-border/50">
              <CardHeader className="pb-2 pt-3 px-3">
                ...
                {totalDia > 0 && (
                  <p className="text-xs text-muted-foreground">{totalDia} un.</p>
                )}
              </CardHeader>
              <CardContent className="px-3 pb-3 space-y-2">
                {diaItens.map(item => (
                  <div key={item.id} className="flex items-center gap-1.5 bg-muted/30 rounded-lg p-1.5">
                    <div className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: getSaborColor(getSaborNome(item.sabor_id)) }} />
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
              <div className="bg-primary/5 rounded-lg p-3 text-center">
                ...
                <p className="text-xs text-muted-foreground">Total da semana (un.)</p>
              </div>
              <div className="bg-primary/5 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-primary">{Object.keys(totalPorSabor).length}</p>
                <p className="text-xs text-muted-foreground">Sabores planejados</p>
              </div>
              <div className="bg-primary/5 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-primary">{DIAS_SEMANA.filter(d => (itensPorDia[d.value] || []).length > 0).length}</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-center py-2 px-1 font-medium text-muted-foreground text-xs">-Vendas (7d proj.)</th>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Semana: {semanaLabel} · {totalGeral} unidades · {Object.keys(totalPorSabor).length} sabores
            </p>
          </div>
          <DialogFooter>
