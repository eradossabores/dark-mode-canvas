...
BEGIN
  ...
  -- Validar e deduzir matéria-prima
  DECLARE
  ...
  -- Inserir produção COM factory_id
  ...
  -- Inserir funcionários responsáveis COM factory_id
  ...
  -- Registrar movimentação COM factory_id
...
-- Also drop the old overload without p_ignorar_estoque to avoid ambiguity
DROP FUNCTION IF EXISTS public.realizar_producao(uuid, modo_producao, integer, integer, text, text, jsonb);
