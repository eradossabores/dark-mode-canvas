...
BEGIN
  ...
  VALUES (p_cliente_id, 0, p_operador, p_observacoes, v_factory_id)
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    IF NOT p_ignorar_estoque THEN
      ...
      IF v_estoque < v_quantidade THEN
        RAISE EXCEPTION 'Estoque insuficiente para sabor %. Disponível: %, Solicitado: %',
          v_sabor_nome, COALESCE(v_estoque, 0), v_quantidade;
      END IF;
    END IF;
    ...
    IF v_estoque IS NULL AND p_ignorar_estoque THEN
      INSERT INTO public.estoque_gelos (sabor_id, quantidade, factory_id)
      VALUES (v_sabor_id, -v_quantidade, v_factory_id)
      ON CONFLICT (sabor_id) DO UPDATE
        SET quantidade = public.estoque_gelos.quantidade - EXCLUDED.quantidade * -1,
            updated_at = now();
      v_rows_updated := 1;
    ELSE
