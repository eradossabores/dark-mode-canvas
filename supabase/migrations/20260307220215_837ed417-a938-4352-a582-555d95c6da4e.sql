DROP FUNCTION IF EXISTS public.realizar_venda(uuid, text, text, jsonb, jsonb, boolean);
DROP FUNCTION IF EXISTS public.realizar_venda(uuid, text, text, jsonb, jsonb);
...
BEGIN
  ...
  -- Calculate total quantity across ALL items for progressive pricing
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    -- Use TOTAL order quantity for price tier calculation
